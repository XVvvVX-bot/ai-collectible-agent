...
def test_interest_digest_report_summarizes_recent_activity_and_ended_comps(tmp_path: Path):
    db_path = tmp_path / "agent_v2.db"
    reports_dir = tmp_path / "reports"
    SqliteV2Store(str(db_path)).ensure_schema()
    now = "2026-03-29T00:00:00+00:00"
    recent = "2026-03-28T23:30:00+00:00"

    with sqlite3.connect(db_path) as conn:
        live_listing_id = _insert_listing(
            conn,
            "L-1",
            title="T69M红楼梦型张新",
            status_raw="2",
        ...
        preview_listing_id = _insert_listing(
            conn,
            "L-2",
            title="T69M红楼梦型张新",
            status_raw="1",
        ...
        conn.execute(
            ...
            VALUES ('u-digest', 'Digest User', 'zh-CN', 'Asia/Shanghai', ?, ?)
            """,
            (now, now),
        )
        conn.execute(
            ...
            INSERT INTO user_profile_defaults_v2 (
              ...
              'u-digest', 'CNY', 'balanced', 'ignore', 'daily_digest', 70, 24, 0, 0, 1,
              'digest defaults', ?, ?
            )
            """,
            (now, now),
        ...
        conn.execute(
            ...
            INSERT INTO user_interests_v2 (
              ...
              'interest-1', 'u-digest', '红楼梦卖点', 'watch_sell', 'exact_item', 'exact',
        ...
        conn.execute(
            ...
            INSERT INTO user_interest_targets_v2 (
              ...
              NULL, '["型张","M"]', '[]', '["新","全品"]',
              NULL, NULL, NULL, 'exact', 'high', 1, ?, ?, 'require'
            )
            """,
            (now, now),
        )
        conn.execute(
            ...
            INSERT INTO user_holdings_v2 (
              ...
              'holding-1', 'u-digest', 'interest-1', 'T69M红楼梦型张新', 'stamp_like', '红楼梦型张',
        ...
        conn.execute(
            ...
            (?, 'u-digest', ?, 'target-1', 'watch', 'exact_identity', 181, 145, 20, 8, 8, 'v2_test', '[]', 'active', ?, ?),
            (?, 'u-digest', ?, 'target-1', 'watch', 'exact_identity', 175, 145, 20, 4, 6, 'v2_test', '[]', 'active', ?, ?)
            """,
            (
                str(uuid.uuid4()),
                live_listing_id,
    ...
    assert result.recent_activity_count == 2
    assert "# V2 Interest Daily Digest" in content
    assert "## Highlights" in content
    assert "Recent activity in lookback window:" in content
    assert "`exact_identity` | `live` | `T69M红楼梦型张新`" in content
    assert "Comp trend: weighted" in content
